	...
	for(var i=0; i<arguments.length; i++){
		...
		*/
		for(var j=0; j<cObj.childNodes.length; j++){
			...
			for(var m=0;m<cObj.childNodes[j].childNodes.length;m++){
				if(cObj.childNodes[j].childNodes[m].nodeName=='#text') continue;
				cObj.childNodes[j].childNodes[m].setAttribute('DragObj1', 'ccDrag');
				}
		}
	}
}
...
function mouseMove(ev){
	...
	*/
	...
	if(typeof(target.DragObj1)!="undefined"&&target.DragObj1=="ccDrag")target=target.parentNode;
	var mousePos = mouseCoords(ev);
