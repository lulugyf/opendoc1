...
uif.validate= { 
    ...
    errMsg: {
	    ...
	    allCellPhone:{//原手机号码验证
	        ...
	        test: function(obj) {
	        	if(!obj.value) return true;
	          var patrn = /^(1(([35][0-9])|(47)|[8][02356789]))\d{8}$/; //modify on 20120809
	           // var patrn = /^1[358][39]\d{8}$/;//电信
	          //var patrn = /^1[35][0126]\d{8}$/;//联通
	          //var patrn = /^1[358][4-9]\d{8}$/;//移动
	            var sInput = obj.value;
	    ...
	    isCharLengthOf: {//需要自定义属性v_maxlength,v_minlength，中文
	        msg: "长度不在指定范围内",
	        test: function(obj) {
	        		if(!obj.value) return true;
	            var maxlen = parseFloat(obj.getAttribute("v_maxlength"));
	            ...
	            var val=obj.value.replace(/(^\s*)|(\s*$)/g, "");
	            if (!isNaN(maxlen)) {
	                if (uif.validate.strlen(val+"") > maxlen) {
	                	this.msg = "长度必须在"+minlen+"到"+maxlen+"字节之间";
	                    return false;
	                }
	            }
	            if (!isNaN(minlen)) {
	                if (uif.validate.strlen(val+"") < minlen) {
	                	this.msg = "长度必须在"+minlen+"到"+maxlen+"字节之间";
	                    return false;
	                }
	            }
	    ...
	    yyyy:{//yyyy
	    	...
	    	test:function(obj){
	    			if(!obj.value) return true;
	    			if(uif.validate.getDateFromFormat(obj.value,"yyyy")==0)
	    					return false;
	    			return true;
	    		} 	
	    	},
	    HHmmss:{//HHmmss
	    	...
	    	test:function(obj){
	    			if(!obj.value) return true;
	    			if(uif.validate.getDateFromFormat(obj.value,"HHmmss")==0)
	    					return false;
	    			return true;
	    		} 	
	    	},
	    yyyyMMddHHmmss:{//yyyyMMddHHmmss
	    	...
	    	test:function(obj){
	    			if(!obj.value) return true;
	    			if(uif.validate.getDateFromFormat(obj.value,"yyyyMMddHHmmss")==0)
	    					return false;
	    			return true;
	    		} 	
	    	},
	  	speyyyyMMddHHmmss:{//yyyyMMddHHmmss
		  	...
		  	test:function(obj){
		  			if(uif.validate.getDateFromFormat(obj.value,"yyyyMMdd HH:mm:ss")==0)
		  					return false;
		  			return true;
		  		} 	
		  	},
	    MM:{//MM
	    	...
	    	test:function(obj){
	    			if(!obj.value) return true;
	    			if(uif.validate.getDateFromFormat(obj.value,"MM")==0)
	    					return false;
	    			return true;
	    		} 	
	    	},
	    dd:{//dd
	    	...
	    	test:function(obj){
	    			if(!obj.value) return true;
	    			if(uif.validate.getDateFromFormat(obj.value,"dd")==0)
	    					return false;
	    			return true;
	    		} 	
	    	},
	  	yyyyMMdd:{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"yyyyMMdd")==0)
	  					return false;
	  			return true;
	  		} 	
	  	},
	  	yyyyMM:{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"yyyyMM")==0)
	  					return false;
	  			return true;
	  		} 		
	  	},
	  	yyyyMMddHH:{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"yyyyMMddHH")==0)
	  					return false;
	  			return true;
	  		}	
	  	},
	  	"lineyyyyMMdd":{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"yyyy-MM-dd")==0)
	  					return false;
	  			return true;
	  		}	
	  	},
	  	"ColonHHmmss":{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"HH:mm:ss")==0)
	  					return false;
	  			return true;
	  		}	
	  	},
	  	"lineyyyyMMddHHmmss":{
	  		...
	  		test:function(obj){
	  			if(!obj.value) return true;
	  			if(uif.validate.getDateFromFormat(obj.value,"yyyy-MM-dd HH:mm:ss")==0)
	  					return false;
	  			return true;
	  		}	
	  	},
		...
		custassure: {
	        ...
	        test: function(obj) {
	        		if(obj.getAttribute("v_flag") == "black")
	        		{
	        			return 0;
	        		}
	        		else if(obj.getAttribute("v_flag") == "idUsed")
	        		{
	        			return 1;	
	        		} 
	            else
	    ...
	    for (var name in errMsg) {
	        ...
	        if (re.test(elem.className)) {
	            ...
	            if (typeof errMsg[name].msg == 'string' && !state) {
	                ...
	                this.showErrors(elem, errMsg[name].msg);
	                break;
	            }
	            else if ((errMsg[name].msg.constructor === Array) && (state !== true)) {
            		if(elem.id!=""){elem.parentNode.errstate=elem.id}
            		else{elem.id=(new Date).getTime();elem.parentNode.errstate=elem.id}
	                this.showErrors(elem, errMsg[name].msg[state]);
	                break;
	            }
	            else {
	    ...
	    for (var i = 0; i < form.elements.length; i++) {
	        ...
	        if(v_display!=0&&form.elements[i].parentNode.errstate&&(form.elements[i].parentNode.errstate==form.elements[i].id)){
		 	...
		 	if(form.elements[i].disabled==false&&v_display==0){
				uif.validate.validateField(form.elements[i]);
			}
			else{
	...
	checksubmit: function(form_01){
		this.errFlag=0;
	    if(typeof form_01 != 'string'){
	        this.checkAll(form_01);
	    }else{
	        this.checkAll(document.getElementsByName(form_01)[0]); 
	    }
	    if(this.errFlag){
	    	return false;
	  	}
	...
	addClass: function(element,value){ //追加样式，而不是替换样式
	 ...
	 disabledError: function(elem){ //当页面上文本框置灰不可用的时候，隐藏其置灰前的验证错误信息
...
$(document).ready(function () {
    ...
    for (var j = 0;j < forms.length;j++){
	    for (var i = 0; i < forms[j].elements.length; i++) {
	       ...
	       $(forms[j].elements[i]).bind("blur",function(){uif.validate.validateField(this);});
	    }
    }
  	//为所有文本框添加特殊字符校验，
	...
	for(var i=0;i<inputs.length;i++){
		if(inputs[i].type=="text"){
			uif.validate.addClass(inputs[i],"haveSpeForAll");
		}
	}
	
	var txtArea = document.getElementsByTagName("textarea");
	for(var i=0;i<txtArea.length;i++){
		uif.validate.addClass(txtArea[i],"haveSpeForAll");
	}
});
