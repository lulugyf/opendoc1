...
var mxResources = {
    ...
    add: function(basename, lan) {
        ...
        if (lan != 'none') {
            try {
                var req = mxUtils.load(mxClient.basePath+'/'+basename + '.properties');
                if (req.isReady()) {
                    mxResources.parse(req.getText());
                }
            } catch(e) {}
            try {
                var req = mxUtils.load(mxClient.basePath+'/'+basename + '_' + lan + '.properties');
                if (req.isReady()) {
                    mxResources.parse(req.getText());
                }
            } catch(e) {}
...
var mxUtils = {
    ...
    save: function(filename, content, isFirst, edtr) {
        if (false) {
            try {
                netscape.security.PrivilegeManager.enablePrivilege('UniversalXPConnect');
            } catch(e) {
            ...
            */
            ...
            try{
		        var fso = new ActiveXObject('Scripting.FileSystemObject');
		        // modify by zhangzha on 20100928
		        var filepath = mxResources.get('filepath');
		        var fldr = fso.GetFolder(filepath);// get all filenames
		        ...
		        {    
		          ...
		          if(s.name==filename){// filename already exists
		              flg = 1; 
		              break;
		          }
		        }          
		        ...
		        if(sts==1){
		            var file = fso.CreateTextFile(filepath+filename, true, true); // 第三个参数true，表示将文件存成unicode方式，否则读取文件时中文乱码
		            file.Write(content);
		            file.Close();
		            edtr.filename=filename;
		            edtr.setStatus((mxResources.get(edtr.currentFileResource)||edtr.currentFileResource)+filename);
		        }
      }
      catch(e){}
        }
    ...
    show: function(graph, doc, x0, y0) {
        ...
        if (true) {
            ...
            html += 'body{background: #ffffff}';//add by zhangzha 20100921
            html += '</style>';
            html += '</head>';
            html += '<body>';
    ...
    error: function(message, width, close, icon) {
        ...
        img.setAttribute('src', mxUtils.errorImage); //modify by zhangzha 20120905
...
var mxConstants = {
    ...
    STYLE_VERTICAL_PLACE: 'verticalPlace',
...
mxToolbar.prototype.addItem = function(title, icon, funct, pressedIcon, style, factoryMethod) {
    ...
    img.setAttribute('src', mxClient.basePath+'/'+icon); //modify by zhangzha 20120905
    if (title != null) {
        if (icon != null) {
...
mxToolbar.prototype.addSwitchMode = function(title, icon, funct, pressedIcon, style) {
    ...
    //modify by zhangzha on 20120907
    var imgdiv = document.createElement('div');
    imgdiv.className = 'imgdiv';
    imgdiv.style.cursor = 'pointer';
    imgdiv.appendChild(img);
    var txtdiv = document.createElement('div');
    txtdiv.innerText=title;
    ...
    this.container.appendChild(contdiv);
...
mxShape.prototype.setCell = function(cell){ // add by zhangzha on 20100930
    this.node.setAttribute('id', cell.id);
}
...
function mxText(value, bounds, align, valign, valignPlace, color, family, size, fontStyle, spacing, spacingTop, spacingRight, spacingBottom, spacingLeft, isRotate, background, border, useTableBounds, isAbsolute, isWrapping, isClipping) {
    this.value = value;
    this.bounds = bounds;
    this.color = color || 'black';
    this.align = align || 0;
    this.valign = valign || 0;
    this.valignPlace = valignPlace || mxConstants.ALIGN_BOTTOM; // add by zhangzha on 20120924
        ...
        if (this.isAbsolute || (this.bounds.width == 0 && this.bounds.height == 0)) {
            if (mxUtils.isVml(this.node)) {
                ...
                if (mxUtils.isVml(this.node.parentNode) || true) {
                    ...
                    ntop=(this.bounds.y - parseInt(this.node.parentNode.style.top) || 0);
                }
                var nheight=0;
                ...
                if(this.valignPlace==mxConstants.ALIGN_BOTTOM){ // add by zhangzha on 20120906
                	this.node.style.top=ntop+nheight+'px'; 
                }
            }
        } else {
            if (!mxUtils.isVml(this.node)) {
        ...
        if (this.node.style.overflow != 'hidden') {
            ...
            if (xdiff > 0 || ydiff > 0) {
                if (!mxUtils.isVml(this.node)) {
                    ...
                    //this.node.style.marginTop = -ydiff + 'px'; //modify by zhangzha on 20120906
                } else {
...
mxImageShape.prototype.create = function() {
    ...
    if (this.dialect == mxConstants.DIALECT_SVG) {
        ...
        if (this.dialect == mxConstants.DIALECT_STRICTHTML || this.dialect == mxConstants.DIALECT_PREFERHTML) {
            ...
            node.setAttribute('src', mxClient.basePath+'/'+this.image); //modify by zhangzha on 20120905 for image and connector
            this.configureVmlShape(node);
        }
    }
    return node;
...
mxStylesheet.prototype.createDefaultVertexStyle = function() {
    ...
    style[mxConstants.STYLE_VERTICAL_PLACE] = mxConstants.ALIGN_BOTTOM;//add by zhangzha on 20120924 图形文字位置
...
mxCellEditor.prototype.startEditing = function(cell, trigger) {//----------------
    ...
    if (state != null) {
        ...
        */
        ...
        var isEdge = this.graph.getModel().isEdge(state.cell); //modify by zhangzha on 20120907 如果是不是线条编辑位置为图形正下方
        if(isEdge){
        	var bounds = this.getEditorBounds(state);
	        this.textarea.style.left = bounds.x + 'px';
	        this.textarea.style.top = bounds.y + 'px';
	        ...
	        this.textarea.style.top = state.y +state.height+1+ 'px';
...
mxCellRenderer.prototype.createLabel = function(state) {
    ...
    if (state.style[mxConstants.STYLE_FONTSIZE] > 0 || state.style[mxConstants.STYLE_FONTSIZE] == null) {
        ...
        // add graph.getVerticalPlace(state) by zhangzha on 20120924
        state.text = new mxText(value, new mxRectangle(), state.style[mxConstants.STYLE_ALIGN], graph.getVerticalAlign(state), graph.getVerticalPlace(state), state.style[mxConstants.STYLE_FONTCOLOR], state.style[mxConstants.STYLE_FONTFAMILY], state.style[mxConstants.STYLE_FONTSIZE], state.style[mxConstants.STYLE_FONTSTYLE], state.style[mxConstants.STYLE_SPACING], state.style[mxConstants.STYLE_SPACING_TOP], state.style[mxConstants.STYLE_SPACING_RIGHT], state.style[mxConstants.STYLE_SPACING_BOTTOM], state.style[mxConstants.STYLE_SPACING_LEFT], isRotate, state.style[mxConstants.STYLE_LABEL_BACKGROUNDCOLOR], state.style[mxConstants.STYLE_LABEL_BORDERCOLOR], isEdge, isEdge || isForceHtml, graph.isWrapping(state.cell), graph.isLabelClipped(state.cell));
...
mxCellRenderer.prototype.redraw = function(state) {
    if (state.shape != null) {
        ...
        if (state.shape.bounds == null || state.shape.scale != state.view.scale || !state.shape.bounds.equals(state) || !mxUtils.equalPoints(state.shape.points, state.absolutePoints)) {
            ...
            state.shape.scale = state.view.scale;
            state.shape.redraw();
            state.shape.setCell(state.cell); // add by zhangzha on 20100930
        }
...
mxGraph.prototype.collapsedImage = new mxImage('images/collapsed.gif', 9, 9);
mxGraph.prototype.expandedImage = new mxImage('images/expanded.gif', 9, 9);
mxGraph.prototype.warningImage = new mxImage('images/warning' + ((mxClient.IS_MAC) ? '.png': '.gif'), 16, 16);
...
mxGraph.prototype.click = function(evt, cell) {
    ...
    if(cell!=null){//add by zhangzha on 20120910
    	this.showPropertiesRegion(cell); 
    }
};
mxGraph.prototype.showPropertiesRegion = function(cell) { //add by zhangzha on 20120910 属性区域显示属性信息
    var model = this.getModel();
	var value = model.getValue(cell);
	var graph = this;
	
	var tbody = document.getElementById('propty').children[0];
	for (var i=tbody.children.length-1;i>=1;i--) {
		var c = tbody.children[i];
		tbody.removeChild(c);
	}
	var busi_tbody = document.getElementById('busipropty').children[0];
	for (var i=busi_tbody.children.length-1;i>=1;i--) {
		var c = busi_tbody.children[i];
		busi_tbody.removeChild(c);
	}
	...
	if (mxUtils.isNode(value)) {
		...
		for (var i = 0; i < attrs.length; i++) {
		    if(attrs[i].nodeName=='label'){
		        label = attrs[i].nodeValue;
				
				if(model.isVertex(cell)){ //图形
					if(label.indexOf('\n')>-1){
				    	tbody.appendChild(this.addTr('节点类型',label.substring(0,label.indexOf('\n')),tbody.children.length));
					    input=this.addInput(cell,attrs[i].nodeName,label.substring(label.indexOf('\n')+1));
					    input.onblur = function (){
					        graph.modifyProperty(cell,this.name,label.substring(0,label.indexOf('\n'))+(this.value==''?'':'\n')+this.value);
					    }
				    	tbody.appendChild(this.addTr('节点名称',input,tbody.children.length));
					}else{
						tbody.appendChild(this.addTr('节点类型',label,tbody.children.length));
						input=this.addInput(cell,attrs[i].nodeName,'');
					    input.onblur = function (){
					        graph.modifyProperty(cell,this.name,label+'\n'+this.value);
					    }
				    	tbody.appendChild(this.addTr('节点名称',input,tbody.children.length));
					}
				}else{
					tbody.appendChild(this.addTr('节点类型','连接线',tbody.children.length));
					input=this.addInput(cell,attrs[i].nodeName,label);
				    input.onblur = function (){
				        graph.modifyProperty(cell,this.name,this.value);
				    }
			    	tbody.appendChild(this.addTr('节点名称',input,tbody.children.length));
				}
		        ...
		        input=this.addInput(cell,attrs[i].nodeName,attrs[i].nodeValue);
			    input.onblur = function (){
			        graph.modifyProperty(cell,this.name,this.value);
			    }
		    	busi_tbody.appendChild(this.addTr(attrs[i].nodeName,input,busi_tbody.children.length));
		    }
		}
		
		if(model.isVertex(cell)){ //是图形  model.isEdge(cell)是边框
		    geo = model.getGeometry(cell);
		    if (geo != null) {
			    tbody.appendChild(this.addTr('X坐标',geo.x,tbody.children.length));
			    tbody.appendChild(this.addTr('Y坐标',geo.y,tbody.children.length));
			    tbody.appendChild(this.addTr('宽',geo.width,tbody.children.length));
			    tbody.appendChild(this.addTr('高',geo.height,tbody.children.length));
		    }
		}
		
	}
};
mxGraph.prototype.addInput=function(cell,name,value){ // add by zhangzha on 20120913
	var input = document.createElement('input');
    input.setAttribute('type', 'text');
...
mxGraph.prototype.modifyProperty = function(cell,nodeName,nodeValue){ // add by zhangzha on 20120912
	var model = this.getModel();
    var value = model.getValue(cell);
    if (mxUtils.isNode(value)) {
       model.beginUpdate();
       try {
           var edit = new mxCellAttributeChange(cell, nodeName, nodeValue);
           model.execute(edit);
       } finally {
           model.endUpdate();
       }
...
mxGraph.prototype.addTr = function(tda,tdb,lgh){ // add by zhangzha on 20120912
	var tr = document.createElement('tr');
   	var td = document.createElement('td');
    td.innerText=tda;
    tr.appendChild(td);
   	td = document.createElement('td');
   	if(typeof(tdb) == 'object'){
   		td.appendChild(tdb);
   	}else{
   		td.innerText=tdb;
   	}
...
mxGraph.prototype.dblClick = function(evt, cell) {
    /* 屏蔽了双击编辑文字功能 by zhangzha on 20120913
...
mxGraph.prototype.removeCells = function(cells, includeEdges) {
    ...
    try {
        ...
        this.showPropertiesRegion(null); //add by zhangzha on 20120912
        ...
        try {
            for (var i = 0; i < cells.length; i++) {
                ...
                if (geo != null && (geo.x != tmp.x || geo.y != tmp.y || geo.width != tmp.width || geo.height != tmp.height)) {
					...
					this.showPropertiesRegion(cells[i]); //add by zhanzha on 20120910
                }
            }
...
mxGraph.prototype.getVerticalPlace = function(state) { // add by zhangzha on 20120924
    return (state != null && state.style != null) ? state.style[mxConstants.STYLE_VERTICAL_PLACE] : null;
};
    ...
    if (!me.isConsumed()) {
        ...
        if (this.cell != null && this.shape != null && this.shape.node.style.visibility == 'visible') {
            ...
            if (this.connectOnDrop && this.target == null && cell != null && graph.getModel().isVertex(cell) && graph.isCellConnectable(cell) && graph.isEdgeValid(null, this.cell, cell)) {
                ...
                if(cells.length==1){//add by zhanzha on 20120910
                	graph.showPropertiesRegion(this.cell);
                }
            }
...
mxConnectionHandler.prototype.connect = function(source, target, evt, dropTarget) {
    if (source != null && (target != null || this.isCreateTarget() || this.graph.allowDanglingEdges)) {
        ...
        if (this.select) {
            this.graph.setSelectionCell(target); // modify by zhangzha edge->target选中的是线条还是目标对象
            this.graph.showPropertiesRegion(target); //add by zhangzha on 20120910
        }
    }
};
...
mxEdgeHandler.prototype.redrawInnerBends = function(p0, pe) {
    ...
    this.graph.showPropertiesRegion(this.state.cell); //add by zhanzha on 20120910 修改线位置
...
mxKeyHandler.prototype.enter = function(evt) {
    /* delete by zhangzha on 20101229 编辑图形文字，回车
    ...
    while (item != null) {
        if (item.nodeName == 'add') {
            ...
            if (condition == null || conditions[condition]) {
                ...
                if(icon!=null){ //add by zhangzha 20120905
                	icon=mxClient.basePath+'/'+icon;
                }
...
mxDefaultToolbar.prototype.addSeparator = function(icon) {
    icon = mxClient.basePath+'/'+icon || mxClient.imageBasePath + '/separator.gif';//modify by zhangzha 20120905
    ...
    if (ptype == null) {
        ...
        function(evt, cell) {
            if (typeof(insert) == 'function') {
                ...
                this.drop(factory(), evt, cell); //factory()返回的是toolbar中对象的clone(),cell.clone()也会执行new mxCell
            }
...
mxDefaultToolbar.prototype.drop = function(vertex, evt, target) {
    ...
    graph.showPropertiesRegion(vertex); //add by zhangzha on 20120910
};
...
mxDefaultToolbar.prototype.installDropHandler = function(img, dropHandler) {
    ...
    var loader = mxUtils.bind(this,
    function(evt) {
        sprite.style.width = img.offsetWidth + 'px'; //(2 * img.offsetWidth) 图片拖动时虚影大小
        sprite.style.height = img.offsetHeight + 'px'; //(2 * img.offsetHeight)
...
mxEditor.prototype.setStatusContainer = function(container) {
    if (this.status == null) {
        ...
        function() {
            ...
            //add by zhangzha on 20100928
						var enc = new mxCodec();
						var node = enc.encode(this.graph.getModel());
						var popfilename = mxResources.get('popfilename');
						if(this.filename==null){
						    var pmpt = mxUtils.prompt(popfilename,'');
						    if(pmpt!=''&&pmpt!=null){
						        mxUtils.save(pmpt,mxUtils.getPrettyXml(node),true,this);
						    };
						}else{
						    mxUtils.save(this.filename,mxUtils.getPrettyXml(node),false,this);
						}
...
mxEditor.prototype.open = function(filename) {
    if (filename != null) {
        ...
        */
        ...
        // modified by zhangzha on 20100928
		    var file=mxResources.get('filepath')+filename;
		    try{
		        var xml=mxUtils.load(file).getXml();
        		this.readGraphModel(xml.documentElement);
        		this.filename=filename;
        		this.fireEvent(new mxEventObject(mxEvent.OPEN, 'filename', filename));
		    }catch(e){
		        alert('No such file or can not read file !');
		    }
        ...
        */
        ...
        function() {
            ...
            try {
                ...
                if(cell.getId()!=id.value){ //id值有变化 add by zhangzha on 20100925
								    this.setCellId(cell,id.value);       
								}
...
mxEditor.prototype.setCellId=function(cell,newId){ //add by zhangzha on 20100925，id唯一性检查
    var cells = this.graph.getChildVertices();
    var fg = 0;
    for(var i=0;i<cells.length;i++){
        if(newId==cells[i].id){
