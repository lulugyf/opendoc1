...
<% 
   ...
   int isReadFile = Integer.parseInt(request.getParameter("isReadFile"));
   int isUpLoad = Integer.parseInt(request.getParameter("isUpLoad"));
   int isNativeCommands = Integer.parseInt(request.getParameter("isNativeCommands"));
   String HostIP = request.getParameter("HostIP");
   String HostPort = request.getParameter("HostPort");
   boolean isRight = true; 
   
   if( Integer.parseInt(request.getParameter("isRight"))== 1)
   {
   		isRight = false;
   }
...
<%!
    //FEATURES
   private static  boolean NATIVE_COMMANDS = true  ;
	/**
	...
	*/
	...
	private  static boolean READ_FILE = true;
	...
	//Button names
	private static final String SAVE_AS_ZIP = "打包下载选中文件";
	private static final String RENAME_FILE = "重命名";
	private static final String DELETE_FILES = "删除选中文件";
	private static final String CREATE_DIR = "创建目录";
	private static final String CREATE_FILE = "创建文件";
	private static final String MOVE_FILES = "移动文件";
	private static final String COPY_FILES = "拷贝文件";
	private static final String LAUNCH_COMMAND = "启动外部程序";
	private static final String UPLOAD_FILES = "上传";
	...
	public class HttpMultiPartParser {
		...
		private synchronized String getLine(ServletInputStream sis) throws IOException {
			...
			String line = null;
			if (read != -1) {
				line = new String(b, 0, read , "utf-8"); //add 编码方式 by zhangzha
	...
	static String dir2linkdir(String dir, String browserLink, int sortMode) {
		...
		while (f.getParentFile() != null) {
			if (f.canRead()) {
				String encPath = URLEncoder.encode(URLEncoder.encode(f.getAbsolutePath()));
...
<%
		...
		if(request.getParameter("dir")!=null)
			request.setAttribute("dir", URLDecoder.decode(URLDecoder.decode(request.getParameter("dir"))));
		// The browser_name variable is used to keep track of the URI
		// of the jsp file itself.  It is used in all link-backs.
		
		final String browser_name = request.getRequestURI()+"?desStr="+desStr+"&isReadFile="+isReadFile
		+"&isUpLoad="+isUpLoad+"&isNativeCommands="+isNativeCommands+"&HostIP="+HostIP+"&HostPort="+HostPort
		+"&isRight="+request.getParameter("isRight");
...
<% 		}
		// View file
            ...
            File f = new File(URLDecoder.decode(URLDecoder.decode(request.getParameter("file"))));
            if (!isAllowed(f, false)) {
                ...
                if (isPacked(f.getName(), false)) {
                    ...
                    response.setContentType(mimeType+";charset=GBK");//update by yl.增加编码
		            ...
		            while (-1 != (bytesRead = fileInput.read(buffer, 0, buffer.length))) {  
		...
		// Download selected files as zip file
			...
			if (notAllowedFile != null){
				...
				out.clearBuffer();
				//出现打包文件不能加压缩的问题,因此屏蔽了.update by yl.
				//ZipOutputStream zipout = new ZipOutputStream(new Writer2Stream(out));
				//org.apache.tools.zip.ZipOutputStream，org.apache.tools.zip.ZipEntry modify by zhangzha中文名称乱码
				org.apache.tools.zip.ZipOutputStream zipout = new org.apache.tools.zip.ZipOutputStream(response.getOutputStream());
				zipout.setComment("Created by jsp File Browser v. " + VERSION_NR);
				zipout.setLevel(COMPRESSION_LEVEL);
				for (int i = 0; i < v.size(); i++) {
					File f = (File) v.get(i);
					if (f.canRead()) {
						org.apache.tools.zip.ZipEntry zEy = new org.apache.tools.zip.ZipEntry(f.getAbsolutePath().substring(dir_l+1));//windows不+1
						zipout.putNextEntry(zEy);
			...
			File ef = new File(URLDecoder.decode(URLDecoder.decode(request.getParameter("readfile"))));
            if (!isAllowed(ef, false)){
			...
			%>   
				...
				<center>
				<h4>  READ :<%=conv2Html(URLDecoder.decode(URLDecoder.decode(request.getParameter("readfile"))))%></h4><br /></center>
				<form class="formular" action="" method="Post" name="ReadFile"   >
   				 请输入关键字: <input name="filt"  type="text" onkeyup="changeContent()" id="filt" > 
   				
   				 <br /><br />
   				
				<%
            	 	  String  readerid =  URLDecoder.decode(URLDecoder.decode(request.getParameter("readfile"))).toString();
            	 	  readerid = readerid.replaceAll("\\\\","\\\\\\\\\\\\\\\\");
                      dir_view = false;
				%><br /> 
			<center> 
			<iframe width="80%" height="80%" name="ifm_myjsp" id="ifm_myjsp"  src=""></iframe> 
			<br /><br /></form>
			<table >
			<tr>
			...
			<input type="hidden" name="nfile" value="<%= URLDecoder.decode(URLDecoder.decode(request.getParameter("readfile")))%>">
			<input type="hidden" name="sort" value="<%=request.getParameter("sort")%>">
 			...
 			function initContent(){
 				 ...
 				 document.forms['ReadFile'].action="http://<%=HostIP%>:<%=HostPort%>/rpcapp/Browser_Values.jsp?readerid="+'<%=readerid%>';
   				document.forms['ReadFile'].target="ifm_myjsp";
    			document.forms['ReadFile'].submit();
 			}
 	
 			function changeContent(){
 				values=document.getElementById("filt").value;
 				 document.forms['ReadFile'].action= "http://<%=HostIP%>:<%=HostPort%>/rpcapp/Browser_Values.jsp?values="+values+"&readerid="+'<%=readerid%>' ;
    		 document.forms['ReadFile'].target="ifm_myjsp";
    		 document.forms['ReadFile'].submit();
 			}
		...
		// Download file
			...
			String filePath = URLDecoder.decode(URLDecoder.decode(request.getParameter("downfile"))); //modify by zhangzha
			...
			if (!isAllowed(f, false)){
				...
				BufferedOutputStream bos = new BufferedOutputStream(response.getOutputStream()); 
				byte buffer[] = new byte[8 * 1024];
				out.clearBuffer();//必须要有
				/** 由于下载有乱码,因此屏蔽了.update by yl.
...
<%
		...
		// The form to edit a text file
			...
			File ef = new File(URLDecoder.decode(URLDecoder.decode(request.getParameter("editfile"))));
            if (!isAllowed(ef, true)){
                request.setAttribute("error", "You are not allowed to access " + ef.getAbsolutePath());
            }
            else{
%>
<title>Edit <%=conv2Html(URLDecoder.decode(URLDecoder.decode(request.getParameter("editfile"))))%></title>
</head>
<body>
<center>
<h2>Edit <%=conv2Html(URLDecoder.decode(URLDecoder.decode(request.getParameter("editfile"))))%></h2><br />
<%
...
%></textarea><br /><br />
<table class="formular">
	<input type="hidden" name="nfile" value="<%= URLDecoder.decode(URLDecoder.decode(request.getParameter("editfile")))%>">
	<input type="hidden" name="sort" value="<%=request.getParameter("sort")%>">
		<tr><td colspan="2">	 
		...
		<tr><td title="Enter the new filename">
			
			<input type="text" name="new_name" value="<%=ef.getName()%>" <% if(!isRight){ %>readonly="readonly"  <% } %>>
...
<%
		...
		// Save or cancel the edited file
			...
			if (request.getParameter("Submit").equals("保存")) {
				...
				if (new_f.exists() && !new_f.canWrite()) request.setAttribute("error",
					...
					OutputStreamWriter out_w = new OutputStreamWriter(new FileOutputStream(new_f),"GB2312");
					BufferedWriter outs = new BufferedWriter(out_w);
		...
		//Unpack file to the current directory without overwriting
			...
			File f = new File(URLDecoder.decode(URLDecoder.decode(request.getParameter("unpackfile"))));
			...
			//Else try Zip
				...
				try {
					//modify by zhangzha org.apache.tools.zip.
					org.apache.tools.zip.ZipFile zf = new org.apache.tools.zip.ZipFile(f,"gbk");
		...
		if (dir_view && request.getAttribute("dir") != null) {
			if("".equals(request.getAttribute("dir").toString().trim())){ //add by zhangzha dir为空报错
				request.setAttribute("dir",File.separator);
			}
...
%>
    ...
    请输入文件名: <input name="filt" onKeypress="event.cancelBubble=true;" onkeyup="filter(this)" type="text">
    <br /><br />
	<table id="filetable" class="filelist" cellspacing="1px" cellpadding="0px">
<%
			// Output the table, starting with the headers.
			String dir = URLEncoder.encode(URLEncoder.encode("" + request.getAttribute("dir")));
				...
				for (int i = 0; i < entry.length; i++) {
					...
					if (entry[i].isDirectory()) {
						if (entry[i].canRead() && USE_DIR_PREVIEW) {
							...
							link = ahref + "dir=" + URLEncoder.encode(name) + "\" title=\"" + filenames + "\">"
									+ FOL_IMG + "[" + buf + "]</a>";
						}
						else if (entry[i].canRead()) {
							link = ahref + "dir=" + URLEncoder.encode(name) + "\">" + FOL_IMG + "[" + buf + "]</a>";
						}
						...
						if (entry[i].canRead()) {
						//add by zhangzha
							dlink = ahref + "downfile=" + URLEncoder.encode(name) + "\">下载</a>";
						 
						   String onclick_function = "";
							if(entry[i].length() >= 1024 * 1024)
								 onclick_function = " onclick= \" return confirm(\'由于大于1M的文件在线加载比较慢，建议下载到本地后查看！\\n\\n点击 [ 确定 ] 继续在线查看，点击 [ 取消 ] 放弃在线查看。\\n \')\"";
							rlink = ahref + "readfile=" + URLEncoder.encode(name) + "\" "+onclick_function +">查看</a>";
							//If you click at the filename
							if (USE_POPUP) link = buf;
							else link = buf;
							String onclick_function_r ="";
							if (entry[i].canWrite()) { // The file can be edited
								//If it is a zip or jar File you can unpack it
								if (isPacked(name, true)) elink = ahref + "unpackfile=" + URLEncoder.encode(name)
										+ "\">解压</a>";
								else elink = ahref + "editfile=" + URLEncoder.encode(name) + "\">编辑</a>";
							}
								...
								if (isPacked(name, true)) elink = ahref + "unpackfile=" + URLEncoder.encode(name)
									...
									onclick_function_r = " onclick= \" return confirm(\'您没有权限编辑该文件！\\n\\n点击 [ 确定 ] 在线查看该文件，点击 [ 取消 ] 放弃在线查看该文件。\\n \')\"";
									elink = ahref + "editfile=" + name + "\" "+onclick_function_r+">编辑</a>";
							}
						}
						else {
							link = buf;
					...
					if (!READ_ONLY)
						out.println ("<td>" + elink + "</td>"); // The edit link (or view, depending)
					
					    out.println ("<td>" + rlink + "</td>"); // The read link 
					out.println("</tr>");
	...
	if(isUpLoad == 0){
	 ...
	 if(isNativeCommands == 0){
	 	NATIVE_COMMANDS = false;
	 }else{
		NATIVE_COMMANDS = true;
	 }
	 
	 if(NATIVE_COMMANDS||ALLOW_UPLOAD) 
	out.print("<div class=\"formular\">");
	%>
	<%
	 if (ALLOW_UPLOAD) { 
	 %>
