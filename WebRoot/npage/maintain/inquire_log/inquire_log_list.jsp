			 ...
			 <logic:iterate id="show" name="netCelllist" indexId="i" type="com.sitech.rom.common.dto.RomNetCell">    
		   		<tr>
					...
					<td align="center"><%=StringUtil.safeValue(show.getCellName())%></td>
			   		...
			   		<td align="center">
			   			<input type="button" class="butAmed" name="set"  title="查询" onClick="inquireRow('<%=StringUtil.safeValue(show.getCellId())%>'
...
function inquireRow(cellId,cellName){
	...
	+"&proId=<%=proId%>&opCode=<%=opCode%>";
	 ...
	 parent.openDivWin("<%=request.getContextPath()%>/gotoInquireSyslog_browser.do?"+qryparam,"查看日志->网元："+cellName,"1000","500");
