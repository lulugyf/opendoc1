...
<div class="title"> <div class="text">应用进程脚本列表</div></div>  
<div class="list">
	<table id="mTable">
		<tr>
			<th>产品</th>
		...
		%>
		<si:pagelist name="progresslist" id="sprocess.qryRomScriptsConfigInfo" param="<%=progressBo %>" scope="end" url="progress_list.jsp">
			 <logic:iterate id="show" name="progresslist" indexId="i" type="com.sitech.rom.common.bo.ProgressBo">    
		   		<tr>
					...
					<td><%=StringUtil.safeValue(show.getActionName())%></td>				   		      
			   		...
			   		<td>
			   	  		<!--
			   		    <A class="btn_begin" name="runstop" style="display:none" title="执行" onclick="processStartOrStop('<%=StringUtil.safeValue(show.getScriptsId())%>')"><SPAN></SPAN></A>
			   		 	 -->
						<% if(show.getIsOwn()==0){ %>
...
function editRow(itemId,actionId,scriptsId){
	...
	parent.openDivWin("<%=request.getContextPath()%>/gotoUpdateAppProgConf.do?"+qrystr,"修改应用进程配置信息","800","300");
	parent.refreshByClose();
}
//删除行
function delRow(itemId,actionId,scriptsId,hostName,itemName,actionName){
	var packet = new AJAXPacket("<%=request.getContextPath()%>/delAppProgConf.do");
...
function doDelRow(packet){
	var retCode = packet.data.findValueByName("retCode");
  	...
  	if(retCode=='1'){
  	    parent.parent.showDialog("应用进程配置删除成功！",2);
  	    parent.doSrchSubmit();
  	}else if(retCode=='noright'){
  	    parent.parent.showDialog("您没有权限进行此操作！",0);
  	}else{
  	    parent.parent.showDialog("应用进程配置删除失败！",0);
  	}
}

function processStartOrStop(scriptsId){
	var qryPara="scriptsId="+scriptsId+"&proId=<%=proId%>&opCode=<%=opCode%>&provinceCode=<%=provinceCode%>&tellType=<%=tellType%>";
	parent.openDivWin("<%=request.getContextPath()%>/processStartOrStop.do?"+qryPara,"执行应用进程脚本信息","800","300");
	parent.refreshByClose();
}
