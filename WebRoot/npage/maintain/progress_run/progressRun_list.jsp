...
<script language="JavaScript">
	...
	<%for(int i=0; i<pageSum; i++){%> // 将java二维List数据转化为js二维数组
		  arrPage[<%=i%>] = new Array(); 
	      arrPage[<%=i%>][0]= '<%=((ProgressBo)dataList.get(i)).getProName()%>';
	      arrPage[<%=i%>][1]= '<%=(((ProgressBo)dataList.get(i)).getProvinceName())+(((ProgressBo)dataList.get(i)).getTellName())%>';
	      arrPage[<%=i%>][2]= '<%=((ProgressBo)dataList.get(i)).getHostName()%>';
	      arrPage[<%=i%>][3]= '<%=((ProgressBo)dataList.get(i)).getHostIp()%>';
	      arrPage[<%=i%>][4]= '<%=((ProgressBo)dataList.get(i)).getItemName()%>';
	      var isOwn='';
	      <%
	      ...
	      arrPage[<%=i%>][5]= isOwn;
	      arrPage[<%=i%>][6]= '';
	      arrPage[<%=i%>][7]= '<%=((ProgressBo)dataList.get(i)).getActionArr()%>';
	      arrPage[<%=i%>][8]= '<%=((ProgressBo)dataList.get(i)).getItemId()%>';
	<%}%>
	...
	function dataWrite(tableID,iStartPos,retNum){
	   ...
	   for(var i=0;i<len;i++){
    	...
    	for(var i=0; i<retNum; i++){ 
			...
			arrTd[6].id='td'+arrPage[iStartPos+i][8];
			arrTd[7]=insertTr.insertCell(7);
			var inHtml='';
			var actionTrr='';
			if(arrPage[iStartPos+i][7].length>0)actionTrr=arrPage[iStartPos+i][7].substring(0,arrPage[iStartPos+i][7].length-1);
			var bunArr=actionTrr.split(';');
			for(var m=0;m<bunArr.length;m++){
				inHtml+='<input type="button" name="runstop" style="display:none" onclick="execProcAction(\''+arrPage[iStartPos+i][8]+'\',\''+bunArr[m].split('_')[2]+'\',\''+arrPage[iStartPos+i][1]+'-'+arrPage[iStartPos+i][3]+'\',\''+arrPage[iStartPos+i][4]+'\')" class="b_foot" value="'+bunArr[m].split('_')[1]+'" />';
			}
			inHtml+='<input type="button" name="runstop" style="display:none" onclick="clearProcAction(\''+arrPage[iStartPos+i][8]+'\')" class="b_foot" value="清空状态" />';
			arrTd[7].innerHTML=inHtml;
		}
...
function execProcAction(itemId,scriptsId,hostName,itemName){
	...
	var packet = new AJAXPacket("<%=request.getContextPath()%>/processRun.do");
