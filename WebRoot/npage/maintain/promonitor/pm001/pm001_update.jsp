   ...
   <div id="operation">
	<div id="operation_table">
		<form action="" method="post" name="pmFrom"> 		 
			...
			<input type=hidden name="seqId" id="seqId" value="${romBusicheckConf.seqId}">
					...
					<table>
						<tr>
							...
							<td colspan="3">
								...
								<select name="busiIdentify" id="busiIdentify"  class="required">
									<option value="" ></option>
						...
						<tr id="tr_100" style="display:none;">
							...
							<td colspan="3">
								<input type="text" name="cellName" id="cellName" value="${romNetCell.cellName}" class="required" readonly="readonly" />
						...
						<tr>
							...
							<td>
								<input type="text" name="checkNodes" id="checkNodes" value="${romBusicheckConf.checkNodes}" v_minlength="1" v_maxlength="32" class="required isCharLengthOf"/>
							</td>	
							<th><font color="red">*进程描述：</font></th>
							<td>
								<input type="text" name="childNode" id="childNode" value="${romBusicheckConf.childNode}" v_minlength="1" v_maxlength="32" class="required isCharLengthOf"/>
							</td>				
	 					</tr>						
						...
						<tr id="grantLoginTr" style="display:none">
							...
							<td colspan="3">
								<input type=hidden name="grantLogins" id="grantLogins" value="${romBusicheckConf.grantLogins}"/>
								<input type="text" name="grantLoginNames" id="grantLoginNames" value="${loginsMess}" style="width:450px" readonly="readonly"/>
...
$(document).ready(function () {
	...
	//初始化信息	
	for(var i=0;i<g("busiIdentify").length;i++){//业务类型
		if(g("busiIdentify")[i].value == "${busiIdentify}"){
			g("busiIdentify")[i].selected = true;
			onChangeBusiIdentify();
			break;
		}
	}
	
	if('${romBusicheckConf.isGrant}'==0){ //共享
		$("input[name='isGrant']").each(function(){
	        if($(this).val()==0){
	            $(this).click();
	        }
	    });
	    if($.trim('${romBusicheckConf.grantLogins}')!=''){ //部分共享
...
function doSrchSubmit(){
	...
	document.pmFrom.action='<%=request.getContextPath()%>/pm001Update.do';
