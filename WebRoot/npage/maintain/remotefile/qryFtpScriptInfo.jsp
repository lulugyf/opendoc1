...
function doSrchSubmit(){
    document.forms['srchFrm'].action="<%=request.getContextPath()%>/npage/maintain/remotefile/ftpsitesScriptList.jsp";
    document.forms['srchFrm'].submit();
    unLoading('ajaxLoadingDiv');
}

function choice(scriptId,hostIp,remotePath,siteName){
	//alert(scriptId+hostIp+remotePath+siteName);
    parent.setFtpScript(scriptId,hostIp,remotePath,siteName);
    closeDivWin();
} 
