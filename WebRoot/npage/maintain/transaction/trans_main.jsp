	    ...
	    <div id="operation_table">			
			<div class="input">	
				<table>
					...
					<tr>
						<th>业务功能名称：</th>
						<td><input type="text" name="busiName" id="busiName"/></td>
						<th>交易名称：</th>
						<td><input type="text" name="transName" id="transName"/></td>
					</tr>
					<tr>
...
function doSrchSubmit(){
    document.forms['srchFrm'].action="<%=request.getContextPath()%>/npage/maintain/transaction/trans_list.jsp"+param;
...
function addM(){
	...
	var qryPara='?proId=<%=proId%>&opCode=<%=opCode%>&provinceCode='+provinceCode+'&tellType='+tellType+'&dataFuncCode='+$('#dataFuncCode').val();
	openDivWin("<%=request.getContextPath()%>/gotoAddTransaction.do"+qryPara,"新增交易节点","800","400");
	refreshByClose();
}
//list页面弹出窗口的关闭事件执行
