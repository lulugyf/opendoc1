...
<div class="list">
	...
	<table id="mTable">
		...
		%>
		<si:pagelist name="progresslist" id="sprocessInfo.qryProgInfoConf" param="<%=progressBo %>" scope="end" url="workOrder_list.jsp">
...
function editRow(itemId){
	...
	parent.openDivWin("<%=request.getContextPath()%>/gotoUpdateWorkOrder.do?"+qryPara,"修改工单配置","800","400");
	parent.refreshByClose();
}
//删除行
function delRow(itemId,hostName,itemName){
	var packet = new AJAXPacket("<%=request.getContextPath()%>/delWorkOrder.do");
...
function doDelRow(packet){
	var retCode = packet.data.findValueByName("retCode");
  	...
  	if(retCode=='1'){
  	    parent.parent.showDialog("工单配置删除成功！",2);
  	    parent.doSrchSubmit();
  	}else if(retCode=='noright'){
  	    parent.parent.showDialog("您没有权限进行此操作！",0);
  	}else{
  	    parent.parent.showDialog("工单配置删除失败！",0);
  	}
}
