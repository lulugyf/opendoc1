			...
			<div class="input">	
				<table>
					...
					<tr>
						<th>*网元编号：</th>
							<td>
							  <select name="hlrCode" id="hlrCode" class="required"
					...
					<tr>
					    <th>智能卡号：</th>
						<td>
							<input type="text" name="imsiNo" id="imsiNo" class="numOrLetter"/>
						</td>	
						<th>用户证号：</th>
						<td>
						    <input type="text" name="idNo" id="idNo" class="isDigLengthOf" v_maxlength="11"  v_minlength="11"/>
						</td>
					</tr>
 					<tr>
						<th>*开始时间：</th>
...
function doSrchSubmit(){
    ...
    document.forms['srchFrm'].action="<%=request.getContextPath()%>/npage/maintain/workorderInquiryDetail/workorderInquiryDetail_list.jsp?tableName="+tableName+"&cellCode="+cellCode+"&hlrCode="+hlrCode;
...
function doReport(){
	...
	var qryPara='?proId=<%=proId%>&opCode=<%=opCode%>&provinceCode='+provinceCode+'&tellType='+tellType+'&tableName='+tableName+'&cellCode='+cellCode+'&hlrCode='+hlrCode+'&flag='+flag;
    document.forms['srchFrm'].action="<%=request.getContextPath()%>/womInquiryDetailExport.do"+ qryPara;
