...
<div class="title"> <div class="text">工单脚本列表</div></div>  
<div class="list">
	<table id="mTable">
		<tr>
			<th>产品</th>
		...
		%>
		<si:pagelist name="progresslist" id="sworkorderScript.qryWorkOrderInfo" param="<%=progressBo %>" scope="end" url="workOrderScript_list.jsp">
...
function editRow(itemId,actionId,scriptsId){
	...
	parent.openDivWin("<%=request.getContextPath()%>/gotoUpdateWorkOrderScript.do?"+qrystr,"修改工单信息","800","300");
	parent.refreshByClose();
}
//删除行
function delRow(itemId,actionId,scriptsId,hostName,itemName){
	var packet = new AJAXPacket("<%=request.getContextPath()%>/delWorkOrderScript.do");
...
function doDelRow(packet){
	var retCode = packet.data.findValueByName("retCode");
  	...
  	if(retCode=='1'){
  	    parent.parent.showDialog("工单删除成功！",2);
  	    parent.doSrchSubmit();
  	}else if(retCode=='noright'){
  	    parent.parent.showDialog("您没有权限进行此操作！",0);
  	}else{
  	    parent.parent.showDialog("工单删除失败！",0);
  	}
}

function runScript(scriptsType,scriptsId,taskId,hostName){

	if(scriptsType=="0"){//定时执行的脚本
		var packet = new AJAXPacket("<%=request.getContextPath()%>/exceWorkOrderScriptByAuto.do");
		packet.data.add("scriptsId" ,scriptsId);
		...
		var qrystr="scriptsId="+scriptsId+"&proId=<%=proId%>&opCode=<%=opCode%>&provinceCode=<%=provinceCode%>&tellType=<%=tellType%>&hostName="+encodeURI(hostName);	
		parent.openDivWin("<%=request.getContextPath()%>/exceWorkOrderScriptByManual.do?"+qrystr,"执行工单","800","300");
		parent.refreshByClose();	
	}
}
...
//停止脚本
function stopScript(scriptsType,scriptsId,taskId,hostName){
		var packet = new AJAXPacket("<%=request.getContextPath()%>/stopWorkOrderScriptByAuto.do");
		packet.data.add("scriptsId" ,scriptsId);
		packet.data.add("taskId" ,taskId);
		packet.data.add("proId" ,"<%=proId%>");
		packet.data.add("opCode" ,"<%=opCode%>");
