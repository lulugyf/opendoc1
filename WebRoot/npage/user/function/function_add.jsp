...
<body>
<div id="operation">
	<form name="addFrm" target="ifm" method=post>
			...
			<div class="input">	
				<table>
					<tr>
						...
						<td>
						    <input type="text" name="functionCode" id="functionCode" class="required isCharLengthOf numOrLetter" v_maxlength="8" v_minlength="0"/>
						</td>
						<th><font color="red">*功能名称</font></th>
						<td>
							<input type="text" name="functionName" id="functionName" class="required isCharLengthOf" v_maxlength="64" v_minlength="0"/>
						</td>
					</tr>
					<tr>
					...
					<tr>
					    <th>显示顺序</th>
						<td>
							<input type="text" name="noteSeq" id="noteSeq" class="nonNegInt"/>
						</td>
                        <th>打开方式：</th> 
                        <td>
					...
					<tr>
					    <th>功能链接</th>
						<td colspan="3">
							<input type="text" style="width:60%" name="actionName" id="actionName" class="isCharLengthOf urlForInput" v_maxlength="256" v_minlength="0"/>
						</td>
					</tr>
					
					<tr>
						<td colspan="4" style="text-align:center">
							<input type="button" class="b_foot" value="确定" onclick="doAddSubmit()"/>&nbsp;&nbsp;
							<input type="button" class="b_foot" value="重置" onclick="javascript:document.forms('addFrm').reset();"/>
					  </td>
					</tr>
				</table>
				<div align="center">
...
function doAddSubmit(){
	...
	if($('#checkParent').attr("checked")==true){
		...
		parentCode=$('#parentCode').val().substring($('#parentCode').val().indexOf(' ')+1);
	}
    ...
    var packet = new AJAXPacket("<%=request.getContextPath()%>/functionAdd.do");
...
function doAddBack(data){
	if(data=='0'){
	    $('#operInfo').html('功能新增成功！');
	    parent.setValue($('#functionCode').val());
	    parent.getFunctionTree(); //刷新功能树
	}else if(data=='1'){
	    $('#operInfo').html('功能新增异常，请正确填写相关信息！');
	}else if(data=='repeat'){
	    $('#operInfo').html('功能新增异常：该功能编码已存在！');
