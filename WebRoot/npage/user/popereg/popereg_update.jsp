...
function updatePoperegSubmit(){
	...
	var packet = new AJAXPacket("<%=request.getContextPath()%>/updatePopereg.do");
	packet.data.add("functionCode" ,$('#functionCode').val());
	packet.data.add("actionName" ,$.trim($('#actionName').val()));
	packet.data.add("functionCodeOld" ,$('#functionCodeOld').val());
	packet.data.add("actionNameOld" ,$('#actionNameOld').val());
	packet.data.add("operationCode" ,$('#operationCode').val());
	packet.data.add("proId" ,"<%=proId%>");
	packet.data.add("opCode" ,"<%=opCode%>");
	packet.data.add("functionLabelOld" ,'${popereg.functionName} ${popereg.functionCode}');
...
function doUpdateBack(data){
	if(data=='0'){
	    ...
	    $('#operInfo').html('更新异常，请正确填写相关信息！');
	}else if(data=='2'){
	    $('#operInfo').html('更新异常，该功能对应的功能连接已经存在！');
